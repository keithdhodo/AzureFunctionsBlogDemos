...
{
    public static class WQUWPCTopicTrigger
    {
        [FunctionName("WQUWPCTopicTrigger")]
        public static void Run(
            [ServiceBusTrigger(Constants.Chapter01.AzureServiceBusTopicNameChapter01, 
                Constants.Chapter01.Chapter01WeightedQuickUnionWithPathCompression, 
                AccessRights.Manage, 
                Connection = Constants.AzureServiceBusConnectionString)]
            MergingArray myQueueItem,
            TraceWriter log,
            [Table(Constants.Chapter01.AzureTableStorageChapter01, Connection = Constants.AzureWebJobsStorageConnectionString)] IAsyncCollector<MergePerformance> outputTable,
            IBinder binder)
        {
            ...
            myQueueItem.Merge(myQueueItem, MergeAlgorithms.WeightedQuickUnionWithPathCompression);

            stopwatch.Stop();

            var performance = new MergePerformance();
            performance.Runtime = stopwatch.Elapsed;
            performance.AlgorithmName = Constants.Chapter01.Chapter01WeightedQuickUnionWithPathCompression;
            performance.PartitionKey = Constants.Chapter01.Chapter01WeightedQuickUnionWithPathCompression;
