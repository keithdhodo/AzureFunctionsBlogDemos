...
{
    public class TribonacciLinearTrigger
    {
        public static void Run(Tribonnaci myQueueItem, TraceWriter log, IAsyncCollector<TribonacciPerformance> outputTable,
            IBinder binder)
        {
            log.Info("TribonacciLinearTrigger processed a request.");

            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Start();

            var outputArray = myQueueItem.Tribonacci(myQueueItem, Enums.TribonacciAlgorithms.linear);
            ...
            var blobPath = "tribonacci" + "/" + "tribonaccilinear" + DateTime.UtcNow.ToString("yyyyMMddHHmmss") + ".txt";
